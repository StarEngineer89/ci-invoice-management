...
class Report extends MX_Controller{
    ...
    function rep_project_child_filter(){
        ...
        if($this->input->post('show')){
            ...
            $data['list']=$this->mdl_extra_report->getOrphanChildName($psite_id, $status_id);
            $this->load->view('rep_project_child_pdf',$data);

        }elseif($this->input->post('xls')){
            $data['list']=$this->mdl_extra_report->getOrphanChildName($psite_id, $status_id);
            $this->load->view('rep_project_child_xls',$data);
        }
    }
    function rep_project_child_payment(){
        $page_details['page_title']="Ucare Foundation | Report Project Child Payment";
        $this->load->module('header')->index($page_details);
        $this->load->view('rep_project_child_payment');
    }
    function rep_project_child_filter_payment(){
        $this->load->model('mdl_extra_report');
        
        $psite_id=$this->input->post('payment_projectsite');
        $orphan_id=$this->input->post('orphanChildName');
        $fromDate=$this->input->post('fromDate');
        $toDate=$this->input->post('toDate');
        if($this->input->post('show')){
            $data['list']=$this->mdl_extra_report->getOrphanPaymentlist($psite_id, $orphan_id, $fromDate, $toDate, '');
            $data['fromDate']=$this->input->post('fromDate');
            $data['toDate']=$this->input->post('toDate');
            $this->load->view('rep_project_child_payment_result',$data);
        }elseif($this->input->post('pdf')){
            $data['list']=$this->mdl_extra_report->getOrphanPaymentlist($psite_id, $orphan_id, $fromDate, $toDate, '');
            $data['fromDate']=$this->input->post('fromDate');
            $data['toDate']=$this->input->post('toDate');
            $this->load->view('rep_project_child_payment_pdf',$data);
        }elseif($this->input->post('xls')){
            $data['list']=$this->mdl_extra_report->getOrphanPaymentlist($psite_id, $orphan_id, $fromDate, $toDate, '');
            $data['fromDate']=$this->input->post('fromDate');
            $data['toDate']=$this->input->post('toDate');
            $this->load->view('rep_project_child_payment_xls',$data);
        }
    }
     function getOrphanChildNamelist(){
        $this->load->model('mdl_extra_report');
        $id = $this->input->post('orphan_projectsite_id');
        $data=$this->mdl_extra_report->getOrphanChildNamelist($id);
        echo json_encode($data);
    }
    function rep_donor_child_payment(){
        $this->load->model('mdl_extra_report');
        $page_details['page_title']="Ucare Foundation | Report Donor Child Sponsor Payment";
        $data['donor_name_list'] = $this->mdl_extra_report->getdetailsDonorName();
        $this->load->module('header')->index($page_details);
        $this->load->view('rep_donor_child_payment', $data);
    }
    function rep_donor_child_filter_payment(){
        $this->load->model('mdl_extra_report');
        ...
        if($this->input->post('show')){
            $data['fromDate']=$this->input->post('fromDate');
            $data['toDate']=$this->input->post('toDate');
            $data['list']=$this->mdl_extra_report->getdetailsDonorPayment($donor_id, $fromDate, $toDate);
            $this->load->view('rep_donor_child_result',$data);
        }elseif($this->input->post('pdf')){
            $data['fromDate']=$this->input->post('fromDate');
            $data['toDate']=$this->input->post('toDate');
            $data['list']=$this->mdl_extra_report->getdetailsDonorPayment($donor_id, $fromDate, $toDate);
            $this->load->view('rep_donor_child_pdf',$data);

        }elseif($this->input->post('xls')){
            $data['fromDate']=$this->input->post('fromDate');
            $data['toDate']=$this->input->post('toDate');
            $data['list']=$this->mdl_extra_report->getdetailsDonorPayment($donor_id, $fromDate, $toDate);
            $this->load->view('rep_donor_child_xls',$data);
        }
    }
