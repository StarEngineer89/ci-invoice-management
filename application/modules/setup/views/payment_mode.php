...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        ...
        <div class="modal-dialog" >
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h3 id="modalTitle">Add Payment Mode</h3>
                </div>
                <div class="modal-body">
                    <form action="<?php echo base_url()?>setup/add_payment_mode" method="post" id="addNewUserForm" class="form-horizontal">
                ...
                <div class="modal-footer">
                   ...
                   <input type="submit" name='btn_event_submit' id="btn_save" value="Add Payment Mode" class="btn btn-primary"/>
                </div>
                </form> 
            </div>
        </div>
 ...
 <div class="row">
    <?php // visible to only valid users having user rights
        ...
        if($u_rights->u_add == 1){ ?>
        <div class="col-xs-12">
        <a href="#" class="btn btn-primary btn-setting" onclick="add_data()">Add Payment Mode</a>
        </div>
        <?php }
        ?>
    ...
    <div class="box-header well" data-original-title="">
        <h2><i class="glyphicon glyphicon-user"></i> Payment Mode List</h2>
    ...
    <table class="table table-striped table-bordered bootstrap-datatable datatable responsive">
        ...
        <tbody>
        <?php
        foreach ($paymentmode_list as $row) {
            if($row['default'] == 1){
        ...
        ?>
            <tr>                                        
                <td class="center"><?php echo $row['paymentmode_id']?></td>
                <td class="center"><?php echo $row['description']?></td>
                <td class="center"><?php echo $default?></td>
                <td class="center"><?php echo $status?></td>
                <td class="center">
                <a class="btn btn-info btn-setting" href="javascript:void(0)" onclick="edit_data('<?php echo $row["paymentmode_id"]?>')">
                    <i class="glyphicon glyphicon-edit icon-white"></i>
                    Edit
                </a>
                <a class="btn btn-danger" onclick="delete_data(<?php echo $row['paymentmode_id']?>);" href="javascript:void(0);">
...
function delete_data(id){
    ...
    if(r== true){
        $.ajax({
            ...
            type: "post",
            url: "<?php echo base_url()?>"+"setup/delete_payment_mode",
...
function add_data(){
    $("#modalTitle").text('Add Payment Mode');
    $("#btn_save").val('Add Payment Mode');
...
function edit_data(id){
    // alert(id);
    $("#modalTitle").text('Edit Payment Mode');
    $("#btn_save").val('Update Payment Mode');
    $.ajax({
        data:{
        id:id
        },
        type: "post",
        url: "<?php echo base_url()?>"+"setup/get_payment_mode_edit",
        dataType: "json",
        success: function(data) {
